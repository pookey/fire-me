...
const navItems = [
  ...
  { to: '/teachers-pension', label: "Teachers' Pension", icon: (
    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M4.26 10.147a60.438 60.438 0 00-.491 6.347A48.62 48.62 0 0112 20.904a48.62 48.62 0 018.232-4.41 60.46 60.46 0 00-.491-6.347M4.26 10.147a50.636 50.636 0 00-2.658-.813A59.906 59.906 0 0112 3.493a59.903 59.903 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 11-1.5 0 .75.75 0 011.5 0Zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 016.75 15.75v-1.5" />
    </svg>
  )},
  ...
  { to: '/lump-sums', label: 'Lump Sums', icon: (
    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function Layout() {
  ...
  return (
    <div className="grain flex h-screen" style={{ background: 'var(--surface-0)' }}>
      {/* Mobile overlay */}
      {sidebarOpen && (
        <div
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-30 w-60 glass transform transition-transform duration-300 ease-out lg:relative lg:translate-x-0 ${
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar - mobile only */}
        <header
          className="h-14 flex items-center justify-between px-5 shrink-0 lg:hidden"
        ...
        >
          ...
          <span className="font-display font-semibold" style={{ color: 'var(--gold-bright)' }}>FinTrack</span>
          <div className="w-5" />
        </header>
